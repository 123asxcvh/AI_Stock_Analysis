...
更新内容：
- 专注于6种核心市场分析类型：概念板块资金流向、行业资金流向、个股资金流向、涨停股票池、财经新闻舆情、市场赚钱效应
- 实现智能数据采样：个股资金流向100行，7:3采样比例（前70%+后30%），其他数据取完整或最新
...
async def main_async(args):
    ...
    print("📊 异步并发处理已启用")
    print("💾 智能缓存机制已启用")
    print("🎯 智能数据采样已启用（个股资金流向50行，7:3比例；其他数据取完整或最新）")
    ...
    async with AsyncMarketAIAnalyzer() as analyzer:
        ...
        print("   ✅ 智能数据采样（个股资金流向100行，7:3比例；其他数据取完整或最新）")
        print("   ✅ 自动报告整合（已启用）")
        print("   ✅ 综合市场分析生成（已启用）")
        print("   ✅ 异步并发处理")
        print("   ✅ 智能缓存机制")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="市场数据AI分析（优化版本，基于已爬取的数据）\n"
                   f"默认执行{len(supported_types)}种分析类型：概念板块资金流向、行业资金流向、个股资金流向、涨停股票池、财经新闻舆情、市场赚钱效应\n"
